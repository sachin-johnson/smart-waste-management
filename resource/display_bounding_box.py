...
def detect_motion(roi, first_frame, threshold=50):  # Increased threshold for better accuracy
    ...
    return contours, cv2.contourArea(thresh)
...
while cap.isOpened():
    ...
    for i, (x, y, w, h) in enumerate(bounding_boxes):
        ...
        cv2.putText(frame, bin_labels[i], (x, y-10), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2, cv2.LINE_AA)
    
    # Display the frame
    cv2.imshow('Bin Detection with Bounding Boxes', frame)
